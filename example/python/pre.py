...
from ase.neighborlist import neighbor_list  # Use neighbor_list for correct cutoff handling
...
def build_neighbor_list_consistent(atoms: Atoms, rc: float) -> Dict[str, torch.Tensor]:
    ...
    # 3. 邻居列表构建 (使用 neighbor_list 函数确保正确的截断距离)
    i_idx, j_idx, S = neighbor_list('ijS', atoms, rc, self_interaction=False)

    # 转换为 Tensor
    center_indices = torch.tensor(i_idx, dtype=torch.long, device=DEVICE)
    neighbor_indices = torch.tensor(j_idx, dtype=torch.long, device=DEVICE)
    shifts = torch.tensor(S, dtype=torch.float32, device=DEVICE)
...
def process_single_frame(atoms: Atoms, model: MagneticNEP, desc_gen: MagneticACEDescriptor, frame_idx: int, filename: str):
    ...
    # 导出描述符用于对比
    np.savetxt('python_descriptors.txt', descriptors.detach().cpu().numpy(), fmt='%.10f')
